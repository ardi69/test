...
if [ ! -f $builddir/_mintlib-header-installed ]; then
	...
	cd include
	$MAKE CROSS=yes prefix=$prefix/$target features.h || { echo "Error Can't create features.h"; exit 1; }
	$MAKE CROSS=yes prefix=$prefix/$target linker.h || { echo "Error Can't create linker.h"; exit 1; }
	cd ..
	$MAKE CROSS=yes prefix=$prefix/$target all-here || { echo "Error Can't all-here"; exit 1; }
