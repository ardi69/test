...
ISL_VER=0.20
ISL_ARC=$(echo -e "#define ISL_CTX_H\n#include <isl/ctx.h>" | gcc -E - &> /dev/null || echo "isl-$ISL_VER.tar.bz2")
ISL_URL="http://isl.gforge.inria.fr/$ISL_ARC"
ISL_SRC="isl-$ISL_VER"

ICONV_VER=1.15
ICONV_ARC=$(echo -e "#define _LIBICONV_H\n#include <iconv.h>" | gcc -E - &> /dev/null || echo "libiconv-$ICONV_VER.tar.gz")
...
MINTLIB_VER="240419"
MINTLIB_GITHUB_ID="dca328840d8bafc91a913356a0a752f6642d1e7e"
MINTLIB_ARC="mintlib-$MINTLIB_VER.tar.gz"
MINTLIB_URL="https://github.com/freemint/mintlib/archive/$MINTLIB_GITHUB_ID.tar.gz"
MINTLIB_SRCX="mintlib-$MINTLIB_GITHUB_ID"
...
which patch >/dev/null 2>&1 || { echo "Can't find patch. Please Install e.g. pacman -S patch"; exit 1; }
which flex >/dev/null 2>&1 || { echo "Can't find flex. Please Install e.g. pacman -S flex"; exit 1; }
which cmp >/dev/null 2>&1 || { echo "Can't find cmp. Please Install e.g. pacman -S diffutils"; exit 1; }
